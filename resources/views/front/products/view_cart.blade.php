            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (data.success) {
                        ...
                        sweetAlert('Increased One Quantity');
                    }
                }
            });
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (data.success) {
                        ...
                        sweetAlert('Decreased One Quantity');
                    }
                }
            });
        }
                        ...
                        <table class="table">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <p class="red">${{$item->unit_price}}</p>
                                    </td>
                                    <td>
                                        <div class="quantity">
                                            <div class="custom">
                                                <button onclick="decrease({{$item->id}})" class="reduced items-count" type="button"><i class="icon_minus-06"></i></button>
                                                <input type="text" name="qty" id="sst_{{$item->id}}" maxlength="12" value="{{$item->quantity}}" title="Quantity:" class="input-text qty">
                                                <button onclick="increase({{$item->id}})" class="increase items-count" type="button"><i class="icon_plus"></i></button>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <p id="price_{{$item->id}}">{{$item->sub_total}}</p>
                                    </td>
                                </tr>
                                @endforeach
                                <tr>
